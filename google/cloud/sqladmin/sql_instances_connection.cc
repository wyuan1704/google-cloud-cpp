...
#include "google/cloud/sqladmin/internal/sql_instances_stub_factory.h"
...
MakeSqlInstancesServiceConnection(Options options) {
  internal::CheckExpectedOptions<CommonOptionList, GrpcOptionList,
                                 SqlInstancesServicePolicyOptionList>(options,
                                                                      __func__);
  options =
      sqladmin_internal::SqlInstancesServiceDefaultOptions(std::move(options));
  auto background = internal::MakeBackgroundThreadsFactory(options)();
  auto stub = sqladmin_internal::CreateDefaultSqlInstancesServiceStub(
      background->cq(), options);
  return std::make_shared<sqladmin_internal::SqlInstancesServiceConnectionImpl>(
...
MakeSqlInstancesServiceConnection(std::shared_ptr<SqlInstancesServiceStub> stub,
                                  Options options) {
  options = SqlInstancesServiceDefaultOptions(std::move(options));
  auto background = internal::MakeBackgroundThreadsFactory(options)();
  return std::make_shared<sqladmin_internal::SqlInstancesServiceConnectionImpl>(
      std::move(background), std::move(stub), std::move(options));
}
